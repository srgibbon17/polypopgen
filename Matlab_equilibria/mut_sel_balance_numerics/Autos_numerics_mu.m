...
s_val = 1e-6; % constant s value

mu_init_val = 1e-8; % initial value of mutation rate
mu_step_size = 1e-8; % change in mu for each iteration/step
...
s_init_str = strcat('s: ', string(s_val));
...
mu_str = strcat('initial mu: ',string(mu_init_val));
a_str = strcat('alpha: ',string(a_val));

parameters_str = {'Parameters:', s_init_str, mu_str, mu_step_size_str, iterations_str, h1_str, h2_str, h3_str, a_str};
dim = [0.5 0.5 0.3 0.3];
...
xscale log
title('Autos: Allele Frequency vs. Mutation Rate')
