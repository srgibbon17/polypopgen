...
syms g00 g01 g10 g11 q pa pb qa qb s mu H D G F d p
...
% equations to model dominance with a power law/hill equation
h1 = (3^-d)/(3^-d + (p/(1-p))^d);
h2 = 1/(1 + (p/(1-p))^d);
h3 = (3^d)/(3^d + (p/(1-p))^d);
% equations to add selection and meiosis
...
Y = solve(mut_eqn_set(1), mut_eqn_set(2), mut_eqn_set(4), q, 'ReturnConditions', true, MaxDegree=4)
