function stable_data = auto_bifn_data_stable_only(s_val_range, mu_val, nu_val, h1_val, h2_val, h3_val, alpha_val)
...
alpha = sym('alpha'); % alpha; double reduction rate
...
sel_g0 = G0*w0+(1/2 + alpha/4)*G1*w1 + (1/6 + alpha/3)*G2*w2 + (alpha/4)*G3*w3;
sel_g1 = (1/2 - alpha/2)*G1*w1 + (2/3 - 2*alpha/3)*G2*w2 + (1/2 - alpha/2)*G3*w3;
sel_g2 = (alpha/4)*G1*w1 + (1/6 + alpha/3)*G2*w2 + (1/2 + alpha/4)*G3*w3 + G4*w4;
...
for i = 1:length(mut_exp_set_eval)
    mut_exp_set_eval(i) = auto_diff_eq_eval(mut_exp_set(i), mu, mu_val, nu, nu_val, h1, h1_val, h2, h2_val, h3, h3_val, alpha, alpha_val);
end
...
for i = 1:length(jac_matrix_eval)
    for j = 1:length(jac_matrix_eval)
        jac_matrix_eval(i, j) = auto_pd_evaluation_init(jac_matrix_eval(i,j), mu, mu_val, nu, nu_val, h1, h1_val, h2, h2_val, h3, h3_val, alpha, alpha_val);
    end
end
...
function [diff_eqn_eval] = auto_diff_eq_eval(diff_eqn, mu, mu_val, nu, nu_val, h1, h1_val, h2, h2_val, h3, h3_val, alpha, alpha_val)
    ...
    diff_eqn_eval = subs(diff_eqn_eval, alpha, alpha_val);
...
function [pd_value] = auto_pd_evaluation_init(jacobian_entry, mu, mu_val, nu, nu_val, h1, h1_val, h2, h2_val, h3, h3_val, alpha, alpha_val)
    ...
    pd_value = subs(pd_value, alpha, alpha_val);
