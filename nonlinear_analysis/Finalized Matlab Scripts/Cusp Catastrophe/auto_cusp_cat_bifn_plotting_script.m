...
s_val_range = [logspace(-9, -8, 10), logspace(-8, -5, 90), logspace(-5, -4, 10)];
k_val_range = [linspace(0, .01, 15), linspace(.01, .05, 25), linspace(.05, .25, 25), linspace(.25, 1, 35)];

k_bistable_range = [linspace(0, .01, 15), linspace(.01, .05, 25), linspace(.05, .25, 25)];

[updated_s_val_range] = updated_s_grid(s_val_range, mu_val, nu_val, k_bistable_range, a_val);

disp('New s grid complete.')

[neutral_q, selected_q, unstable_q, neutral_avg_fitness, selected_avg_fitness, unstable_avg_fitness, s_coord, k_coord] = auto_cusp_cat_data(updated_s_val_range, mu_val, nu_val, k_val_range, a_val);
