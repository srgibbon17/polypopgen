function [neutral_F, neutral_s, selected_F, selected_s, unstable_F, unstable_s] = auto_F_stat_bifn_data(s_val_range, mu_val, nu_val, h1_val, h2_val, h3_val, a_val)
...
for i = 1:length(s_val_range)
    ...
    if g1_root_vals(j) ~= 0 && g0_root_vals(j) ~= 0
...
neutral_p = 1 - neutral_q;
selected_p = 1 - selected_q;
unstable_p = 1 - unstable_q;

neutral_F = (-neutral_g1 + 2*neutral_p.*neutral_q)./(2*neutral_p.*neutral_q);
selected_F = (-selected_g1 + 2*selected_p.*selected_q)./(2*selected_p.*selected_q);
unstable_F = (-unstable_g1 + 2*unstable_p.*unstable_q)./(2*unstable_p.*unstable_q);
