...
for i = 1:length(mu_val)
    for j = 1:length(nu_val)
        for k = 1:length(h_val)
            [g0_bifn_soln, s_bifn_soln] = bifn_numeric_solver(mut_g0==0, derivative_g0==0, mu, mu_val(i), nu, nu_val(j), h, h_val(k), g0, s);
...
g0_bifn_values = [];
s_bifn_values = [];
for m = 1:length(g0_bifn_soln)
    if g0_bifn_soln(m) ~= 0
        g0_bifn_values(end+1) = g0_bifn_soln(m);
        s_bifn_values(end+1) = s_bifn_soln(m);
    end
end
